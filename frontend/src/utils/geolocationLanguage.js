...
const INDIAN_STATE_CODES = {
  ...
  MI: "hi",
  NA: "hi",
  AS: "hi", // Assam - no Assamese translation available, defaulting to Hindi
};
...
const getSupportedLanguages = () => {
  ...
  return i18n.options.supportedLngs || i18nLanguages.length > 0
    ? i18nLanguages
    : [
        "en",
        "de",
...
const isLanguageSupported = (lang) => {
  ...
  if (supportedLanguages.some((supported) => supported.toLowerCase() === lang.toLowerCase())) {
    return true;
  }

  // Language code match (without region)
  const langCode = lang.split("-")[0];
  if (supportedLanguages.some((supported) => supported.toLowerCase().startsWith(langCode.toLowerCase()))) {
    return true;
  }
...
const getBestLanguage = (preferredLang) => {
  ...
  // Case-insensitive match
  const caseInsensitiveMatch = supportedLanguages.find((lang) => lang.toLowerCase() === preferredLang.toLowerCase());
  ...
  const exactMatch = supportedLanguages.find((lang) => lang.toLowerCase().startsWith(langCode.toLowerCase()));
  if (exactMatch) {
    return exactMatch;
  }
...
export const detectAndSetLanguage = async (i18nInstance) => {
  try {
    ...
    const detectedLang = await detectLanguageFromLocation(locationData.country, locationData.region);
